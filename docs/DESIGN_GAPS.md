...
### 3.2 Web-Based Onboarding Flow
PRD §3.3 describes the steps (Google sign-in, calendar-sharing setup, forwarding-email setup, self-introduction form) but there's no system design for the actual onboarding web app — pages, the "check connection" calendar-sharing verification action, where the self-introduction text is captured and handed to Gemini for the initial Knowledge Graph seed. As of PRD v1.10, this flow no longer involves an OAuth consent screen for Calendar/Tasks (Calendar is sharing-based; Tasks is deferred to Phase 2) — only sign-in OAuth for session identity. This is distinct from `email-onboarding.md`'s email-first flow; the two need a defined relationship (does a user do one, the other, or both? does completing one short-circuit the other?).
...
PRD §12 specs the survey endpoint contract and §13 specs page structure, but there's no design for the `web-server` Cloud Run service itself: routing, session/auth (Google OAuth login + 30-day cookie — sign-in only, distinct from Calendar access which is calendar-sharing based, and from the future per-user Tasks OAuth scopes in Phase 2), how archive (`/archive`) and preferences (`/preferences`) pages query Firestore/Cloud Storage, and the actual implementation of `POST /api/survey/submit` (idempotency keying, async hand-off to `graph_update`).
...
### 3.6 Cloud Infrastructure & Deployment
...
### 3.7 Firestore Schema Consolidation & Isolation Enforcement
