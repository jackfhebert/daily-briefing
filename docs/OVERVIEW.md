...
| Secret Manager | Gemini API key; the bot's own Calendar service-account credentials (NWS needs no key, just a `User-Agent` header); per-user OAuth tokens are a Phase 2+ concern, once Tasks ships |

External integrations: Google Calendar API v3 (read via calendar sharing in Phase 1, not per-user OAuth), Google Tasks API v1 (Phase 2, per-user OAuth), Gmail API (read + send), National Weather Service API (per-user tracked cities, no API key required), and Gemini 1.5 Pro for all LLM work.

## Phased rollout (summary)

1. **Phase 1 — Foundation (MVP):** infra, OAuth sign-in + admin provisioning, calendar-sharing setup (no per-user OAuth), Knowledge Graph seeding, calendar + tracked-cities weather, Gmail inbox parsing + shadow calendar extraction/dedup, morning briefing generator, survey endpoint.
