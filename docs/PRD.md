...
> See also: [Email-Based User Onboarding — System Design](./designs/email-onboarding.md), a separate, email-first intro flow that can build a user's Knowledge Graph asynchronously before or alongside the flow below, and [Web-Based Onboarding & Signup — System Design](./designs/web-onboarding.md), which elaborates the steps below in full and defines the handoff between the two flows.
...
Weather is tracked per city, not per user — each user maintains a list of cities they care about (home, a second home, an upcoming trip destination, where their kids' grandparents live, etc.), mirroring the `forwarding_emails[]` pattern used for the email inbox.

- Source: National Weather Service API (api.weather.gov) — free, no API key required, but requests must include a descriptive `User-Agent` header per NWS usage policy
- Each user record stores `tracked_cities[]`: `{ slug, label, lat, lon, nws_gridpoint }` — `nws_gridpoint` (office + grid X/Y) is resolved once via the NWS `/points/{lat},{lon}` lookup and cached on the city entry, since it rarely changes
- One forecast fetch per tracked city, per user, per day — no per-city Gemini parse is needed; NWS forecast data is already structured
- Morning: today's forecast per tracked city — high/low, precipitation probability, sunrise/sunset
- Evening capture page: tomorrow's forecast for the user's primary (first-listed) city shown as context
- Smart callouts (e.g., flag rain during school pickup window, weekend conditions if it's Friday) are generated by the briefing synthesis call, not at fetch time — they require cross-referencing the forecast against calendar and Knowledge Graph context (school pickup time, weekend plans), which the fetch step doesn't have
- The first tracked city is treated as the user's home city for rhythm/anomaly purposes; additional cities are shown as supplementary context (e.g., "Grandma's house: sunny, 68°F" ahead of a planned visit)
...
- 5 AM cron: enqueue + process calendar/weather/email parse jobs per user
